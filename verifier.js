...
async function showRequest(res) {
  ...
  const requestURL = credentialRequest.authorizationRequestDeeplinkUri
   ...
   if (typeof window.DigitalCredential !== 'undefined') {
    ...
    b.onclick = async function() {
     ...
     try {
      ...
      const data = dcResponse.token || dcResponse.data
   ...
   async function checkStatus() {
    ...
    if (resp.status == 200) {
     ...
     if (status.credentials?.length > 0) {
      ...
      <table>
      <tr><th><span lang="fi">Hetu</span><span lang="en">Personal identifier</span></th><td>\${credential.Person.personal_administrative_number}</td></tr>
      <tr><th><span lang="fi">Eläke voimassa</span><span lang="en">Pension effectual</span></th><td>\${credential.Pension.effectual}</td></tr>
      </table>
